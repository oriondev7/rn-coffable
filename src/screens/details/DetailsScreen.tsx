...
import { View, Text, StatusBar, SafeAreaView, ScrollView, TouchableWithoutFeedback, TouchableOpacity } from 'react-native'
...
export const DetailsScreen = ({ navigation, route }: any) => {
  const item = useStore(
    (state: any) => 
      route.params.type === ItemType.coffee 
      ? state.coffeeList 
      : state.beanList
  )[route.params.index]
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.footer}>
          ...
          {showFullDescription ? (
            <TouchableWithoutFeedback onPress={() => setShowFullDescription(state => !state)}>
              <Text 
            ...
            {item.prices.map((data: any) => (
              ...
              >
                <Text style={[styles.sizeText, {
                  fontSize: item.type === ItemType.bean
                    ? FONT_SIZE.size_14
                    : FONT_SIZE.size_16,
