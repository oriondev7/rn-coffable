import { View, Text, StatusBar, ScrollView, TouchableOpacity, TextInput, FlatList, SafeAreaView, ToastAndroid } from 'react-native'
...
export const HomeScreen = ({ navigation }: any) => {
  ...
  const [categoryNames, setCategoryNames] = useState(categoryNamesFromData(coffeeList))
  const [searchText, setSearchText] = useState('')
  const [category, setCategoryIndex] = useState({
    index: 0,
    name: categoryNames[0]
  })
  const [sortedCoffee, setSortedCoffee] = useState(sortedCoffeeList(category.name, coffeeList))
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
            ...
            >
              <TouchableOpacity
                ...
                onPress={() => { 
                  ...
                  setSortedCoffee([...sortedCoffeeList(categoryNames[index], coffeeList)])
                }}
              >
                <Text style={[
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyList}>
              <Text style={styles.categoryTitleBaseStyle}>No Coffee Available</Text>
            </View>
          }
          showsHorizontalScrollIndicator={false}
          data={sortedCoffee}
          ...
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => navigation.navigate(ScreenName.details, { 
                index: item.index,
                id: item.id,
                type: item.type
            })}>
        ...
        <FlatList 
          ...
          showsHorizontalScrollIndicator={false}
          data={beanList}
          contentContainerStyle={[styles.flatListContainer, { marginBottom: bottomTabBarHeight }]}
          keyExtractor={item => item.id}
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => navigation.navigate(ScreenName.details, { 
                index: item.index,
